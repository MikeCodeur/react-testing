  ...
  rest.post(
    ...
    async (req, res, ctx) => {
      if (!req.body.password) {
        return res(ctx.status(400), ctx.json({errorMessage: 'le password est obligatoire'}),ctx.delay(delay))
      }
      if (!req.body.username) {
        return res(ctx.status(400), ctx.json({errorMessage: 'le username est obligatoire'}),ctx.delay(delay))
      }
